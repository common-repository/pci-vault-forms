...
Securely [capture payment card data](https://docs.pcivault.io/developers/capturing-card-data/) from your site using PCI Vault. PCI Vault is a vendor neutral PCI DSS compliant environment designed to reduce your PCI compliance scope to a SAQ by using [credit card tokenization](https://pcivault.io).

Data captured with this plugin will be sent to PCI Vault's DSS compliant environment directly, and will not be present on your own server. This allows you to securely capture and [tokenize credit card](https://pcivault.io) data without being PCI DSS compliant.
